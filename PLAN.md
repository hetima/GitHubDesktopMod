...
- `desktop/out/renderer.js` -> `<version>/renderer.js`
- `desktop/out/renderer.css` -> `<version>/renderer.css`
...
`desktop` で production build する。yarn が存在しなかったら corepack yarn で実行する。
`cross-env` が見つからない場合は依存関係が未導入なので、frozen lockfile で導入してから再実行する。

```powershell
cd desktop
corepack yarn compile:prod

# cross-env が見つからない場合のみ
corepack yarn install --frozen-lockfile
corepack yarn compile:prod
```

成功後、`app/package.json` の `version` からコピー先を決定し、フォルダを作成して生成物をコピーする。
以下はリポジトリルートで実行する。

```powershell
$version = node -p "require('./desktop/app/package.json').version"
New-Item -Path $version -ItemType Directory -Force | Out-Null
Copy-Item -LiteralPath "desktop\out\renderer.js" -Destination "$version\renderer.js" -Force
Copy-Item -LiteralPath "desktop\out\renderer.css" -Destination "$version\renderer.css" -Force
```
